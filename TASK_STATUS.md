...
37. **✅ Done** — QR sheet QR codes now fit their cards: the `.qr-card` grid column is bound with `grid-template-columns: minmax(0, 1fr)` (previously the implicit `auto` column was sized by the URL text's max-content, so the QR's fixed `max-width: 190px` overflowed the card), the QR scales to the card content (`max-width: 100%`), and QR URLs wrap (`overflow-wrap: anywhere`); sheet grid min column raised to 180px for larger printable codes. E2E regression-guarded (every QR must sit inside its card box)
...
49. **✅ Done** — "Add restaurant" moved from Home to the Setup page; Home now has a separate **Order history** panel for served/rejected/cancelled orders (same layout as Live orders, expandable summaries, no dismiss)
50. **✅ Done** — Closed-table visit separation verified by E2E: after serving the last order and closing a table's dining session, a brand-new customer at the same table (fresh device) gets a new sequential order number + a new tracking token — never a continuation of the previous visit (new ACTIVE session is created; partial unique index enforces one active session per table)
51. **✅ Done** — Home dashboard layout: Live tables, Order history and Team now sit side by side (responsive `.panel-trio` grid); Live orders gained a **List / By status / By table** grouping toggle (persisted in `localStorage`); every live order row has a **Manage in Orders** deep link that opens the workspace scrolled to that order card (`?restaurantId=…&focus=<trackingToken>` + `.focus-flash` highlight)
52. **✅ Done** — Greek translation completeness: the Setup console (CSV import, item editor, category/price/status/actions labels, placeholders, "Save changes", "— Unassigned —"), the SUPER_ADMIN Platform Analytics console (fully wired through `useLanguage`), the admin "All restaurants"/"Unknown user" strings, and the home "Unknown table" fallback are all translated (EN+EL)
53. **✅ Done** — Live-orders reconciliation groundwork: shared `src/lib/orders/status.ts` (OPEN_STATUSES / HISTORY_STATUSES / STATUS_TABS / STATUS_OPTIONS / CLOSED_STATUSES / STATUS_PRIORITY) now drives both the home dashboard and the Orders workspace, and the home order fetch limit matches the workspace (200)
54. **✅ Done** — Shared realtime data layer: `src/lib/hooks/use-live-orders.ts` now owns the orders fetch (one `LIVE_ORDERS_SELECT` + limit 200), the `orders` + `order_items` realtime subscriptions, and the refresh for BOTH the home dashboard and the Orders workspace. Component-specific UI (new-order banners, line-draft sync) runs via callbacks before the unified refresh; the workspace re-initializes drafts once per restaurant (guarded effect) so realtime refreshes never clobber in-progress edits
55. **✅ Done** — **Order again / repeat** (EN+EL):
   - **Customer:** the tracking page has an **"Order again"** button that opens the table menu with the previous order's items pre-filled in the cart (quantities, matched modifiers, notes; unavailable/removed menu items are skipped). The customer keeps all or removes some before submitting — a fresh order (new number + tracking token). Powered by `menu_item_id` now exposed by the order-status API and the `?reorder=<trackingToken>` param on `/t/[token]`. E2E-covered
   - ~~Staff "+ {item}" repeat chips~~ — **rolled back** per request (staff can already modify existing orders)
56. **✅ Done** — **Staff live-orders bug fixed**: single-restaurant staff members never saw live orders because `selectedRestaurantId` was only set when there was more than one restaurant option. The home dashboard now always publishes the resolved restaurant id (drives `useLiveOrders`). Live-verified with a staff login + fresh order
57. **✅ Done** — Home dashboard reordered to stacked full-width panels: **Team → Live orders → Order history → Live tables** (was a 3-column trio)
58. **✅ Done** — **Names for accounts**: `restaurant_users.full_name` (migration 014, resilience fallback in code) is set at registration, "add another restaurant" and team invites; shown in the home Team panel, the Team page (add-member form has a Name & surname field) and the "Handled by / Handling" labels — no more emails as names
...
70. **✅ Done** — **Orders workspace compactness + new-order alerts**:
   - **Compact order items**: each line is now a two-row block — `qty × name · modifiers` with the line total on top, and an inline row with a small qty input, note input/button and compact Save/Remove buttons (new `.order-line*`, `.input-sm`, `.button-sm` styles). Same edit capabilities, ~⅓ the vertical space. E2E regression test added
   - **"Order again" in Greek** is now literally `παραγγελία ξανά` (was `Ξανά παραγγελία`)
   - **Tab-title badge**: on any new order for the active restaurant, the browser tab shows `(1) 🔔 New order — <title>` (count accumulates); clears on focus, visibility, or any interaction. Works without notification permission
...
76. **✅ Done** — The card counter is a **pure quantity selector** (default `1`, `−` disabled at 1): `+`/`−` only set how many of that item will be added — they never touch the cart. **"Add to cart" commits the selected quantity at once** (badge + cart get N), then the selector resets to `1`. The cart panel keeps its own steppers for adjusting the cart directly. E2E covers select-3 → add → badge `3` → select-2 → add → badge `5` — **24/24 E2E, 66/66 unit tests green**
77. **✅ Done** — **Chime fixed + verifiable, back-to-top added**: the new-order sound was failing because browsers only play audio after a user gesture. The `AudioContext` is now re-armed on every kind of interaction (pointerdown/mousedown/keydown/touchstart/click), `resume()` is awaited before scheduling the notes so they aren't dropped on a suspended clock, and playback is wrapped in try/catch. A **"Test sound" button** (🔊) next to the Orders sound toggle and Home Live-orders header lets staff verify the chime (a click also unlocks audio for the realtime alert). The **Orders page now has a floating ↑ back-to-top button** that appears after scrolling and smooth-scrolls to the top. E2E: new "back-to-top" test + probe verified the chime gesture-unlock (`AudioContext.state === 'running'`) — **25/25 E2E, 66/66 unit tests green**
78. **✅ Done** — **Live orders vs Order history in the workspace**: the "Live orders" area now shows **only open orders** (NEW, ACCEPTED, PREPARING, READY) — its tabs are ALL/NEW/ACCEPTED/PREPARING/READY. **Terminal orders (SERVED, CANCELLED, REJECTED) move to a new "Order history" panel** below (collapsed by default with a count, expandable, with All/Served/Cancelled/Rejected filter tabs and per-order expandable summaries). **Closing a table session now completes its still-open orders** (marks them SERVED + status-history entries) so they leave live orders immediately. Shared `src/lib/orders/status.ts` updated (`STATUS_TABS` = live-only, new `HISTORY_TABS`). E2E: new "terminal orders leave live orders and move to history" + "closing a table session completes its open orders" tests; existing session/status tests updated — **27/27 E2E, 66/66 unit tests green**
79. **✅ Done** — **"Sign in" double-click fixed**: two root causes addressed in the login flow. (1) **Browser autofill**: controlled inputs often miss autofill because it doesn't fire React `onChange`, so the first submit could send empty credentials — the form now reads email/password straight from `FormData` (uncontrolled inputs). (2) **App Router navigation race**: `router.replace()` was called before `router.refresh()`, letting a cached RSC payload drop the first navigation — now `router.refresh()` runs first, plus a hard `window.location.assign` fallback if still stuck on `/login` after 1.5s. Same refresh-before-navigate ordering applied to password reset and nav sign-out. Verified via probe that autofill-style DOM fill logs in on ONE click — **27/27 E2E, 66/66 unit tests green**
80. **✅ Done** — **Back-to-top + Home Live tables assignment/close**:
   - **↑ back-to-top**: extracted into a shared `BackToTop` component, now on BOTH the Orders workspace AND the Home dashboard, and it appears after a smaller scroll (200px) so it's reachable on shorter pages too
   - **Direct table assignment on Home**: owners/managers/platform-admins now get an **Assign table to ▾ (staff picker) + Assign** control on every Live table card (plus the existing "Claim table" for self, and plain staff keep Claim only)
   - **Close table on Home**: each occupied table gets a **Close table** button — it closes the table's active session, **marks all its open orders as completed (SERVED)** so they leave live orders, and the **next order on that table starts a brand-new visit** (new session/customer). Confirm dialog + notice included
   - E2E: new "home live tables let the owner assign staff and close a table" test + existing back-to-top test — **28/28 E2E, 66/66 unit tests green**
81. **✅ Done** — **Claim removed + back-to-top everywhere**: the "Claim table" action is **removed entirely** — every role now assigns via the **Assign ▾ picker**, which always lists the signed-in user first as **"Me"** (so owners can assign to themselves) followed by the team. The ↑ back-to-top moved into the **AppShell**, so it appears on **every page and every role** (Home, Orders, Setup, Team, Settings, Analytics, Admin, auth pages, and the **customer ordering page** — where it stacks above the floating cart). Debugging note: Next.js injects a route-announcer element between the page `<main>` and the button, so the cart-clearance CSS uses the general sibling combinator (`main.ordering-page ~ .up-float`). E2E: new customer back-to-top test + "Claim is gone" assertion — **29/29 E2E, 66/66 unit tests green**
82. **✅ Done** — **Menu editing, table rooms, table colors, and a real setup bug fixed**:
   - **Per-category "Add item"**: each category header now has an **Add item** button next to Collapse/Rename/Delete that opens an inline data-entry row (name, price, description) with **Save changes** — the item is inserted at the end of that category
   - **Drag & drop reordering**: menu item rows are draggable (⠿ handle); dropping reorders within the category and persists via renumbered `sort_order` (customer menu already orders by `sort_order`, so the new order applies immediately)
   - **Tables per room**: new migration **016** adds `restaurant_tables.room`; the owner gives each table a room when creating it (e.g. Indoors/Outdoors/Terrace) and Setup groups tables under room headings. Code is **resilient pre-migration** (PGRST204/42703 fallback) — rooms just group under "General" until migration 016 is applied
   - **Home Live tables colors**: each table card is now fully colored — **teal** when free, **#E73671** when occupied — with a **Free/Occupied label pinned top-right**
