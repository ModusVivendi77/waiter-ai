...
76. **✅ Done** — The card quantity stepper is now **always visible** (defaults to `1` with `−` disabled) next to "Add to cart", so customers see the quantity control before their first add; `+` on the default adds the item (with the toast), `+` after that increments, `−` decrements to `0` (removing the item — the counter then returns to its default `1`). "Add to cart" remains as an alternative. E2E covers the full 1→2→1→0(default 1) cycle — **24/24 E2E, 66/66 unit tests green**
...
**Last commit:** feat — menu-card quantity stepper defaults to 1
