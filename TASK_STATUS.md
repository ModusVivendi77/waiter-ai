...
76. **✅ Done** — The card quantity stepper is now **always visible** (starts at `0` with `−` disabled) next to "Add to cart", so customers see the quantity control before their first add; `+` on `0` adds the item (with the toast), `+` after that increments, `−` decrements to `0` (removing the item). "Add to cart" remains as an alternative. E2E covers the full 0→1→2→1→0 cycle — **24/24 E2E, 66/66 unit tests green**
...
**Last commit:** feat — always-visible quantity stepper on menu cards (starts at 0, minus disabled)
